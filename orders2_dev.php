...
dojo.xhrGet({
    ...
    <?php }else{ ?>
		url:"http://www.kerussods.com/ajax_controller.php?action=get_onhand2&size="+sizeText+"&pCode="+productCode+"&kdssid=<?php echo $_GET['kdssid'];?>",
    <?php } ?>
...
dojo.xhrGet({
    ...
    <?php }else{ ?>
		url:"http://www.kerussods.com/ajax_controller.php?action=get_sizes2&pCode="+product_code+"&kdssid=<?php echo $_GET['kdssid'];?>",
    <?php } ?>
...
var orderIsReady = true;
    ...
    if( document.forms[0].customer_country.options[CountrySelectedIndex].value != 'United States' ){
...
//*************************************************************************
    ...
    $ord_inventory_sql = "SELECT distinct products2_model, products2_description, products2_color FROM products2 p2 WHERE 1 ORDER BY products2_model";
    ...
    while($ord_inventory = my_db_fetch_array($ord_inventory_query)){
        $productText =$ord_inventory['products2_model']." / ".
        stripslashes(substr($ord_inventory['products2_description'], 0, 50))." / ".
        stripslashes(substr(str_replace("'","",$ord_inventory['products2_color']), 0, 20));
        $arrInventory[] = array('id' => $ord_inventory['products2_model'],
                          'text' => $productText);
    }
    ...
    $ord_countries_sql = "SELECT countries_name, countries_iso_code_3 FROM countries order by countries_name";
    $ord_countries_query = my_db_query($ord_countries_sql);
    $countries_default = "United States";
    while($ord_countries = my_db_fetch_array($ord_countries_query)){
        $arrCountries[] = array('id' => $ord_countries['countries_name'],
                          'text' => $ord_countries['countries_name']);
    }


?>
<?php echo my_draw_form('add_order',my_href_link('orders2_dev.php', 'action=ord_add','POST',' id="add_order"'));?>
...
// **************************************************************************
 ...
 //                   $arrPrices[$ord_sizes['sizes_name']] =  $ord_sizes['sizes_fee'];
                ...
                (%s, %s, %s, %s, %s, %s, %s, %s, %s, %d, %s, '".date("y-m-d h:i:s")."', %s, %s, %s, %d, %01.2f,%01.2f,%d, %d, %s)",
				...
				$ord_inventory_sql = "SELECT * FROM products2 WHERE 1 ORDER BY products2_model";
				$ord_inventory_query = my_db_query($ord_inventory_sql);
				while($ord_inventory = my_db_fetch_array($ord_inventory_query)){
					$productText = " [ ".$ord_inventory['products2_model']." ] ".
					substr($ord_inventory['products2_description'], 0, 20);
					$arrInventory[ $ord_inventory['products2_model'] ] = $productText;
					$arrProductModelToGroupId[$ord_inventory['products2_model']] = $ord_inventory['products2_price_group_id'];
				}

				$price_level_discount_sql = "SELECT pl.price_level_discount, a.accounts_price_level FROM price_levels pl, accounts a WHERE a.accounts_number =  '$accounts_number' AND a.accounts_price_level = pl.price_level_id";
				$price_level_discount_query = my_db_query($price_level_discount_sql);
				$arr_price_level_discount = my_db_fetch_array($price_level_discount_query);
				$percent_discount = ($arr_price_level_discount["price_level_discount"]/100);
				$account_price_level = $arr_price_level_discount["accounts_price_level"];


                $newOrderStatusID = 10;
                $order_history_sql = "INSERT into orders_history (order_id, order_history_date, order_history_status)
                VALUES ('$ord_add_insert_id','".date("y-m-d h:i:s")."', $newOrderStatusID)";
                ...
                for($i=0; $i<$order_size; $i++){
		            ...
		            $productModel = trim($_POST['product_name_'.$i]);
		            $generic_size = $arrSizes[$_POST['product_size_'.$i]];


		            if( is_numeric($productModel{3}) ){
		            	$category = trim(substr($productModel, 0, 3));
		        	} else {
		            	$category = trim(substr($productModel, 0, 4));
		        	}
					// If price_level == 0, that is using the custom pricing
					if ($_SESSION['price_level'] == 0){
						$productPrice = getPriceBySize($_POST['accounts_number'],$productModel, $arrSizes[$_POST['product_size_'.$i]]);
					}else{
						$productPrice = getPriceFromModelSize( $productModel, $generic_size, $percent_discount);
					}
                    ...
                    "'".strtoupper($category . " - " .mysql_real_escape_string($generic_size))."'",
...
if( $_SESSION['userlevel'] == 'super'){
	...
	echo "-------------------------------------<br>";
	echo "Old Price System: $" . getPriceBySize($_POST['accounts_number'],$productModel, $arrSizes[$_POST['product_size_'.$i]]);
	echo "<br>";
	echo "New Price using price level discount applied to MSRP: $" .  getPriceFromModelSize( $productModel, $generic_size, $percent_discount);
	echo "<br>";
	echo "Acct Number: " . $_POST['accounts_number'] . "<br>";
	echo "Model: " . $productModel . "<br>";
	echo "Product Model: " . $productModel . "<br>";
	echo "Size: ". $generic_size . "<br>";
	echo "Group ID: " . $arrProductModelToGroupId[$productModel] . "<br>";
	echo "Price Level: " . $account_price_level . "<br>";
	echo "Percent Discount: " . $percent_discount . "<br>";
	echo "MSRP: " . getMSRPPriceFromModelSize( $productModel, $generic_size, $percent_discount);
}
                ...
                if( $ord_add_insert_id != 0 ){
                	...
                	updateIfBulkOrder( $ord_add_insert_id );
                    ...
                    echo "<br><br><br><br> <a href=\"".my_href_link('orders2_dev.php')."\">Submit another order?</a></div>";
